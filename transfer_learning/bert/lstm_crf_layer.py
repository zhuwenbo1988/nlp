...
class BLSTM_CRF(object):
    def __init__(self, embedded_chars, hidden_unit, cell_type, num_layers, dropout_rate,
                 initializers, num_labels, seq_length, labels, lengths, is_training, domain_vector=None):
        """
        BLSTM-CRF 网络
        ...
        self.domain_vector = domain_vector
    ...
    def blstm_layer(self, embedding_chars):
        ...
        with tf.variable_scope('rnn_layer'):
            ...
            if self.domain_vector is not None:
                outputs = tf.concat([outputs, self.domain_vector], 2)
        return outputs
    ...
    def project_bilstm_layer(self, lstm_outputs, name=None):
        ...
        with tf.variable_scope("project" if not name else name):
            with tf.variable_scope("hidden"):
                extra_size = 0
                if self.domain_vector is not None:
                    extra_size = self.domain_vector.shape[-1].value
                    
                W = tf.get_variable("W", shape=[self.hidden_unit * 2 + extra_size, self.hidden_unit],
                                    dtype=tf.float32, initializer=self.initializers.xavier_initializer())

                b = tf.get_variable("b", shape=[self.hidden_unit], dtype=tf.float32,
                                    initializer=tf.zeros_initializer())
                output = tf.reshape(lstm_outputs, shape=[-1, self.hidden_unit * 2 + extra_size])
