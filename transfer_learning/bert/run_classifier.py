...
class CLSProcessor(DataProcessor):
  """Processor for the Context data set."""
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    lines = self._read_data(os.path.join(data_dir, "dev_data.json"))
    examples = []
    for (i, line) in enumerate(lines):
      guid = "dev-%d" % (i)
      text_a = tokenization.convert_to_unicode(line['text'])
  ...
  def get_test_examples(self, data_dir):
    """See base class."""
#     lines = self._read_data(os.path.join(data_dir, "test_data.json"))
    lines = self._read_data(os.path.join(data_dir, "review.json"))
    examples = []
    for (i, line) in enumerate(lines):
      guid = "test-%d" % (i)
      text_a = tokenization.convert_to_unicode(line['text'])
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if example.label in label_map:
    ...
    print("not existed label = " + example.label)
    raise Exception()
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if FLAGS.save_tf_serving_model:
        gpu_config = tf.ConfigProto()
        gpu_config.gpu_options.allow_growth = True
        sess=tf.Session(config=gpu_config)
        print("load model from:" + FLAGS.output_dir)
        saver = tf.train.Saver()
        saver.restore(sess, tf.train.latest_checkpoint(FLAGS.output_dir))
        tf.saved_model.simple_save(sess,
                                   FLAGS.output_dir + "/tf-serving/",
                                   inputs={"input_ids": input_ids,
                                           "input_mask": input_mask,
                                           "label_ids": label_ids,
                                           "segment_ids": segment_ids},
                                   outputs={"probabilities": probabilities})
        FLAGS.save_tf_serving_model = False
        print("save model to :" + FLAGS.output_dir + "/tf-serving/")
        ...
        #with tf.gfile.FastGFile('output/nlu.pb', mode='wb') as f:
        #    f.write(output_graph_def.SerializeToString())
        #save_path = saver.save(sess, "output/save_net.ckpt")
