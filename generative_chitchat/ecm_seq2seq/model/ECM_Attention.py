...
# ychliu
# 这个ECMWrapperState类就是增加了"internal_memory"用来存储变化的内部情感
class ECMWrapperState(
    collections.namedtuple("ECMWrapperState",
                           ("cell_state", "attention", "internal_memory", "time", "alignments",
                            "alignment_history"))):
    """`namedtuple` storing the state of a `ECMWrapper`.
    Contains:
    ...
    def clone(self, **kwargs):
        ...
        """
        return super(ECMWrapperState, self)._replace(**kwargs)
...
class ECMWrapper(rnn_cell_impl.RNNCell):
    ...
    @property
    def state_size(self):
        """The `state_size` property of `ECMWrapper`.
        Returns:
        ...
        return ECMWrapperState(
            ...
            # ==============================================
            internal_memory=self._emo_internal_memory_units,  # 这里是返回内部情感memory的size
            # ==============================================
    ...
    def zero_state(self, batch_size, dtype):
        """Return an initial (zero) state tuple for this `ECMWrapper`.
        **NOTE** Please see the initializer documentation for details of how
        ...
        with ops.name_scope(type(self).__name__ + "ZeroState", values=[batch_size]):
            ...
            error_message = (
                "When calling zero_state of ECMWrapper %s: " % self._base_name +
                "Non-matching batch sizes between the memory "
    ...
    def call(self, inputs, state):
        ...
        if not isinstance(state, ECMWrapperState):
            raise TypeError("Expected state to be instance of ECMWrapperState. "
