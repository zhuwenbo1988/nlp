...
def _check_maybe(t):
    if isinstance(t, tensor_array_ops.TensorArray):
        raise TypeError(
            "TensorArray state is not supported by ECMBeamSearchDecoder: %s" % t.name)
    if t.shape.ndims is None:
        raise ValueError(
            "Expected tensor (%s) to have known rank, but ndims == None." % t)


class ECMBeamSearchDecoder(decoder.Decoder):
    """BeamSearch sampling decoder.

      **NOTE** If you are using the `ECMBeamSearchDecoder` with a cell wrapped in
...
# ychliu
def _beam_search_step(real_vocab_size, time, logits, next_cell_state, beam_state, batch_size,
    ...
    total_probs = array_ops.expand_dims(
        beam_state.log_probs, 2) + step_log_probs
