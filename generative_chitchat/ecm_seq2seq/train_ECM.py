...
from utils import get_ecm_training_config
from utils import load, save, prepare_ecm_train_dev_data
from utils import create_vocab_tables, create_vocab_file, get_ecm_train_batch
...
def main(args):
    ...
    (train_s_file, train_t_file, train_choice_file, train_category_file,
     dev_s_file, dev_t_file, dev_choice_file, dev_category_file,
     max_length,
     gpu_fraction, gpu_id, checkpoint_every, max_checkpoints, print_every,
     train_steps, is_beam_search, batch_size, beam_size) = get_ecm_training_config(config)
    ...
    train_set, dev_set = prepare_ecm_train_dev_data(train_s_file, train_t_file,
    ...
    try:
        for step in range(saved_global_step + 1, num_steps):
            ...
            if step % print_every == 0 and step != 0:
                # train perplexity
                t_perp = ecm_model.compute_perplexity(sess, batch)
                add_summary(train_writer, step, 'train perplexity', t_perp)

                # eval perplexity
                dev_str = ""
                if dev_set is not None:
                    eval_batch = get_ecm_train_batch(dev_set, max_length, batch_size)
                    eval_perp = ecm_model.compute_perplexity(sess, eval_batch)
            ...
            if step % checkpoint_every == 0:
                save(ecm_model.saver, sess, logdir, step)
