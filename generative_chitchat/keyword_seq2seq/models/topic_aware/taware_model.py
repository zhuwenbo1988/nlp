...
from models.base import AbstractModel, create_attention_mechanism, create_cell
from models.topic_aware import taware_layer, taware_decoder
from utils import print_out
from device import DeviceManager, RoundRobin
...
class TopicAwareSeq2SeqModel(AbstractModel):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope or 'ta_seq2seq_graph', dtype=self.dtype):
            self.init_embeddings(params.vocab_file, params.embedding_type, self.embedding_size, scope=scope)
            ...
            if mode == tf.contrib.learn.ModeKeys.TRAIN:
                ...
                self.word_count = tf.reduce_sum(self.iterator.source_sequence_lengths) +                                   tf.reduce_sum(self.iterator.target_sequence_length)
            ...
            if log_trainables:
                ...
                for trainable in trainables:
                    print_out("  %s, %s, %s" % (trainable.name, str(trainable.get_shape()),
                                                    trainable.op.device))
    ...
    def __build_encoder(self, params, keep_prob):
        with variable_scope.variable_scope("encoder"):
            ...
            if params.encoder_type == "uni":
                print_out("  build unidirectional encoder num_layers = %d" % params.num_layers)
                cell = create_cell(params.cell_type, params.hidden_units, self.num_layers,
                ...
                print_out("  build bidirectional encoder num_layers = %d" % params.num_layers)

                fw_cell = create_cell(params.cell_type, params.hidden_units, num_bi_layers,
                                                  input_keep_prob=keep_prob,
                                                  devices=self.round_robin.assign(num_bi_layers))
                bw_cell = create_cell(params.cell_type, params.hidden_units, num_bi_layers,
                                                  input_keep_prob=keep_prob,
