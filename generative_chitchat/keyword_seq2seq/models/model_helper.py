...
from utils import print_out, safe_exp, print_time
...
def create_or_load_model(model, model_dir, session, name):
    ...
    if latest_ckpt:
        ...
        session.run(tf.tables_initializer())
        print_out("  created %s model with fresh parameters, time %.2fs" %
    ...
    while True:
        try:
            ...
            if step % 500 == 0:
                ...
                print_out("    ## After %d steps, loss %.2f - ppl %.3f" % (step, ls, ppl))
